...
public class MultiplayerVRSynchronization : MonoBehaviour, IPunObservable
{
    ...
    public Transform mainCosmeticAvatar;
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            ...
            stream.SendNext(mainCosmeticAvatar.transform.localRotation.eulerAngles.y);
        ...
        else
        {
            ...
            float newAvatarRotation = (float)stream.ReceiveNext();

            if(remoteSyncBody)
                mainCosmeticAvatar.transform.localRotation = Quaternion.Euler(mainCosmeticAvatar.transform.localRotation.x, newAvatarRotation, mainCosmeticAvatar.transform.localRotation.z);
        }
    }
